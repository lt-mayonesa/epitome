...
examples — get human approval, then create stub `ARCHETYPE.md` files with
`status: discovered`. These stubs are the task-tracking artifacts that `epitome-pin`
will work through one by one.
...
Do **not** write full ARCHETYPE.md content (rules, checklist, anti-patterns, epitome_file)
at this step. That is step 2 (`epitome-pin`).
...
Also skip archetypes already present in `.epitome/` with `status: rejected` — those were
explicitly dismissed in a previous run.
...
CONTROLLER / ENDPOINT
  [1] controller_rest         — REST controller class structure
      83 instances: OrderProcessController.java
  [2] endpoint_get_single     — GET /{pathVar} → single typed resource
      45 instances: StockAnomalyController.java
  ...

SERVICE
  [5] service                 — business logic orchestration layer
      239 instances: OrderlineModificationService.java
  ...

Approve all? Or specify changes (add / remove / rename):
```

**Wait for explicit human approval before creating any files.**
Do not proceed to step 5 until the human confirms the list.

---

### 5. After approval — create stub ARCHETYPE.md files

For each approved archetype:

1. Create the directory: `mkdir -p .epitome/<archetype_id>`
2. Write `.epitome/<archetype_id>/ARCHETYPE.md` with `status: discovered`
...
**Why detected:** <one sentence — what structural marker triggered this seam>

**Instances found:** <N> files matching the detection pattern

**Representative files:**
- `path/to/MostRecent.java` (modified N days ago)
- `path/to/AnotherOne.java` (modified N weeks ago)
- `path/to/ThirdExample.java` (modified N months ago)
```

**Do not** write `epitome_file`, `## What it is`, rules, anti-patterns, or any other
full-archetype content. Those are written by `epitome-pin`.

List each file created as you go so the human can see progress.
...
- `.epitome/<id>/ARCHETYPE.md` created for each approved archetype with `status: discovered`
- No other content written
