...
---
id: <archetype_id>
status: discovered | pinned | reviewed | rejected
...
<!-- Present only when status: discovered. Replaced by full content after pinning. -->
...
| `status` | Yes | Workflow status — see lifecycle below |
| `epitome_file` | Conditional | Relative path (from repo root) to the canonical real file. Required for `pinned` and `reviewed`. Omitted when `status: discovered` or `status: rejected` |
...
|-------|--------|---------|
| `discovered` | `epitome-init` | Seam detected, stub created — waiting to be pinned |
| `pinned` | `epitome-pin` | Epitome file chosen, full ARCHETYPE.md written — ready for review |
| `reviewed` | `epitome-review` | Compared against all real instances, rules and anti-patterns finalised |
| `rejected` | `epitome-pin` | Explicitly rejected during pinning — not a real pattern, already covered by another archetype, or too granular. Kept so future `init` runs do not re-propose it |
